...
// ===== API CALLS =====
// Trong project thực tế, import từ file API:
// import { getUsersPaginatedSearchAPI, addUserAPI, updateUserAPI, deleteUserAPI } from '@/apis/admin.api';

const API_BASE_URL = import.meta.env.VITE_API_URL || 'https://airbnbnew.cybersoft.edu.vn/api';
const TOKEN_CYBERSOFT = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0ZW5Mb3AiOiJCb290Y2FtcCA4MyIsIkhldEhhblN0cmluZyI6IjE4LzAxLzIwMjYiLCJIZXRIYW5UaW1lIjoiMTc2ODY5NDQwMDAwMCIsIm5iZiI6MTc0MTg4ODgwMCwiZXhwIjoxNzY4ODQ1NjAwfQ.rosAjjMuXSBmnsEQ7BQi1qmo6eVOf1g8zhTZZg6WSx4';
...
const getUsersPaginatedSearchAPI = async (pageIndex, pageSize, keywords) => {
  try {
    // Build params giống như API của bạn
    const params = new URLSearchParams();
    params.append('pageIndex', pageIndex.toString());
    params.append('pageSize', pageSize.toString());
    
    // Chỉ thêm keywords nếu có
    if (keywords && keywords.trim()) {
      params.append('keywords', keywords.trim());
    }
    
    const url = `${API_BASE_URL}/users/phan-trang-tim-kiem?${params.toString()}`;
    ...
    // Giống như code của bạn: return response.data.content
    // Nên API response có structure: { statusCode, content: {...}, message }
    ...
    return { pageIndex, pageSize, totalRow: 0, keywords: keywords || null, data: [] };
  }
};
...
const addUserAPI = async (userData) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData?.content || errorData?.message || 'Thêm người dùng thất bại');
  }
...
const updateUserAPI = async (userId, userData) => {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => null);
    throw new Error(errorData?.content || errorData?.message || 'Cập nhật người dùng thất bại');
  }
...
const deleteUserAPI = async (userId) => {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => null);
    throw new Error(errorData?.content || errorData?.message || 'Xóa người dùng thất bại');
  }
